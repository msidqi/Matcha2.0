...
const ImageUpload = () => {
    ...
    const onGalleryClick = ({ target }) => {
        if (target.classList.contains("delete")) {
            ...
            gallery.children.length === 1 && setIsHidden(false)//empty.classList.remove("hidden");
            delete FILES[ou];
            setFILES({ ...FILES })
        }
    };
    ...
    return (
        <>
            <div className="bg-gray-500 h-screen w-screen sm:px-8 md:px-16 sm:py-8">
                <main className="container mx-auto max-w-screen-lg h-full">
                    ...
                    <article aria-label="File Upload Modal" className="relative h-full flex flex-col bg-white shadow-xl rounded-md" onDrop={dropHandler} onDragOver={dragOverHandler} onDragLeave={dragLeaveHandler} onDragEnter={dragEnterHandler}>
                        {/* overlay */}
                        <div id="overlay" ref={overlayRef} className={`w-full h-full absolute top-0 left-0 pointer-events-none z-50 flex flex-col items-center justify-center rounded-md ${isDraggedover && 'draggedover'}`}>
                            <i>
                        ...
                        <section className="overflow-auto p-8 w-full h-full flex flex-col">
                            ...
                            <ul id="gallery" ref={galleryRef} onClick={onGalleryClick} className="flex flex-1 flex-wrap -m-1">
                                <li id="empty" ref={emptyRef} className={`h-full w-full text-center flex flex-col justify-center items-center ${isHidden && 'hidden'}`}>
            ...
            <template id="file-template" ref={fileTempl}>
