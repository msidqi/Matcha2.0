...
const ImageUpload = ({ limit = 0, imagePreviews = [], setImagePreviews }: ImageUploadProps) => {
            ...
            <article aria-label="File Upload Modal" className="relative h-full flex flex-col bg-white shadow-xl rounded-none sm:rounded-md" onDrop={dropHandler} onDragOver={dragOverHandler} onDragLeave={dragLeaveHandler} onDragEnter={dragEnterHandler}>
                ...
                <section className="overflow-auto p-8 w-full h-full flex flex-col">
                    <header className="border-dashed border-2 border-gray-400 py-12 sm:flex flex-col justify-center items-center hidden">
                        ...
                        <input id="hidden-input" onChange={onHiddenChange} ref={hiddenInputRef} type="file" accept="image/*" multiple className="hidden" />
                    ...
                    <ul id="gallery" className="flex flex-1 flex-wrap -m-1">
                        {imagePreviews.map(({ objectURL, ...elem }, index) => (
                            <ImagePreview key={index} {...elem} src={objectURL} onClickDelete={() => handleImagePrevewDelete(index)} />
                        ))}
                        {imagePreviews.length < limit &&
                            <li className="block p-1 w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/6 xl:w-1/8 h-24 sm:hidden" onClick={onButtonClick}>
                                <article tabIndex={0} className="flex justify-center items-center group hasImage w-full h-full shadow-innerRadius rounded-2xl focus:outline-none focus:shadow-outline  cursor-pointer relative text-transparent hover:text-white hover:shadow-sm ">
                                    <img alt="upload preview" src="/add.svg" className="h-1/2 object-cover" />
                                </article>
                            </li>
                        }
